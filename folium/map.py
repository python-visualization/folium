...
class BeautifyMarker(MacroElement):
    ...
    >>> BeautifyMarker(location=[45.5, -122.3], popup=folium.Popup('Portland, OR'),
                       ...
                       background_color='#FFF').add_to(mapa)
    >>> BeautifyMarker(location=[45.5, -122.3], popup=folium.Popup('Portland, OR'),
                       text_color='#000', border_color='transparent',
                       background_color='#FFF', number=10,
                       inner_icon_style='font-size:12px;padding-top:-5px;').add_to(mapa)
>>> BeautifyMarker(location=[45.5, -122.3], popup=folium.Popup('Portland, OR'),
                       icon='arrow-down', icon_shape='marker').add_to(mapa)

    """
    ICON_SHAPE_TYPES = ['circle-dot', 'doughnut', 'rectangle-dot',
                        'marker']

    def __init__(self, location, icon='arrow-down', icon_shape='marker',
